...
import Position6Particles from "./Position6Particles";

/**
 * Position #6 — minimalist progress bar with Entry and Liq. markers.
 ...
 */
const IMG_ENGLAND = "/playground/position-6/england-flag.svg";
const IMG_ARGENTINA = "/playground/position-6/argentina-flag.svg";
const VIDEO_SRC = "/playground/position-6/england.mp4";
  ...
  return (
    <span
      className={`inline-flex items-baseline text-[102px] font-semibold leading-none tracking-tight text-white tabular-nums drop-shadow-[0_4px_24px_rgba(0,0,0,0.55)] ${instrumentSansCondensed.className} ${
        exiting
          ? amount > 0
            ? "position6-size-delta-exit-up"
            : "position6-size-delta-exit-down"
          : "position6-size-delta-enter"
      }`}
    >
      <span>
        {amount > 0 ? "+$" : "-$"}
...
export default function Position6() {
  ...
  const cashOutGlowAnimation =
    cashOutGlow.mode === "liq"
      ? `position6-cash-out-liq-pulse ${cashOutGlow.pulseMs}ms ease-in-out infinite`
      : cashOutGlow.mode === "win"
        ? `position6-cash-out-win-blink ${cashOutGlow.pulseMs}ms ease-in-out infinite`
        : undefined;
  ...
  return (
    <div className="flex h-full w-full items-center justify-center p-4">
        ...
        <div className="absolute inset-x-3 bottom-3 z-10">
          <div
            className={`relative flex w-full flex-col overflow-hidden rounded-[24px] border px-4 pb-4 pt-1 backdrop-blur-xl transition-[background,border-color] duration-500 ease-out ${
              isFastForwardFx ? "animate-[position6-shake_360ms_ease-in-out_1]" : ""
      ...
      <div
        ...
        aria-controls="position6-actions"
      ...
      <div className="flex w-full items-center gap-4">
        ...
        >
          {cashOutGlow.mode !== "off" && (
            <span
              aria-hidden
              className="position6-cash-out-glow pointer-events-none absolute inset-0 rounded-[12px] motion-reduce:opacity-60"
              style={{
      ...
      >
        ...
        <div
          ...
          } ${liqLineProximity > 0.35 ? "position6-liq-line-pulse" : ""}`}
          style={{
      ...
      <style>{`
        ...
        @keyframes position6-cash-out-liq-pulse {
          0%, 100% {
        ...
        @keyframes position6-size-delta-enter {
          0% {
            opacity: 0;
        ...
        @keyframes position6-size-delta-exit-up {
          0% {
            opacity: 1;
            transform: translate3d(0, 0, 0) scale(1);
          }
        ...
        @keyframes position6-size-delta-exit-down {
          0% {
            opacity: 1;
            transform: translate3d(0, 0, 0) scale(1);
          }
        ...
        .position6-size-delta-enter {
          animation: position6-size-delta-enter 420ms cubic-bezier(0.22, 1, 0.36, 1) both;
        }

        .position6-size-delta-exit-up {
          animation: position6-size-delta-exit-up 480ms cubic-bezier(0.4, 0, 0.2, 1) both;
        }

        .position6-size-delta-exit-down {
          animation: position6-size-delta-exit-down 480ms cubic-bezier(0.4, 0, 0.2, 1) both;
        }
