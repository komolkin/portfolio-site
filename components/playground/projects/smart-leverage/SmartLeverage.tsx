...
import SmartLeverageSection, {
  smartLeverageFromIdx,
  SMART_LEVERAGE_STEPS,
} from "@/components/playground/projects/smart-leverage/SmartLeverageSection";
...
const PERSON_OPTIONS = [
  { name: "Timothée Chalamet", image: IMG_TIMOTHEE_CHALAMET, yesPercent: 61, yesLeverage: 3, noLeverage: 2 },
  { name: "Zendaya", image: IMG_ZENDAYA, yesPercent: 67, yesLeverage: 5, noLeverage: 1 },
  { name: "Florence Pugh", image: IMG_FLORENCE_PUGH, yesPercent: 62, yesLeverage: 4, noLeverage: 2 },
  { name: "Paul Mescal", image: IMG_PAUL_MESCAL, yesPercent: 70, yesLeverage: 2, noLeverage: 4 },
  { name: "Saoirse Ronan", image: IMG_SAOIRSE_RONAN, yesPercent: 64, yesLeverage: 3, noLeverage: 5 },
] as const;

function leverageToIdx(leverage: number) {
  return Math.min(4, Math.max(0, leverage - 1));
}
...
const YES_MAX_LEVERAGE_CYCLE_MS = 5000;
...
export default function SmartLeverage() {
  ...
  const [yesMaxLeverage, setYesMaxLeverage] =
    useState<(typeof SMART_LEVERAGE_STEPS)[number]>(1);
  const [yesLeverageIdx, setYesLeverageIdx] = useState(0);
  const [noLeverageIdx, setNoLeverageIdx] = useState(0);
  ...
  const activeOddMaxLeverage =
    outcome === "yes" ? yesMaxLeverage : selectedPerson.noLeverage;
  const leverageIdx = outcome === "yes" ? yesLeverageIdx : noLeverageIdx;
  const handleLeverageIdxChange = useCallback(
    (idx: number) => {
      if (outcome === "yes") setYesLeverageIdx(idx);
      else setNoLeverageIdx(idx);
    },
    [outcome],
  );
  const leverage = smartLeverageFromIdx(leverageIdx);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      setYesMaxLeverage((current) =>
        current >= 5 ? 1 : ((current + 1) as (typeof SMART_LEVERAGE_STEPS)[number]),
      );
    }, YES_MAX_LEVERAGE_CYCLE_MS);

    return () => window.clearInterval(timer);
  }, [outcome]);

  useEffect(() => {
    setNoLeverageIdx((idx) => Math.min(idx, leverageToIdx(selectedPerson.noLeverage)));
  }, [selectedPerson]);
        ...
        <div ref={contentRef} className="relative flex w-full flex-col gap-2.5 p-4">
          ...
          <div className="relative flex h-12 w-full overflow-hidden rounded-full bg-white/[0.04] p-0.5 transition-colors duration-200 hover:bg-white/[0.06]">
            ...
            <button
              ...
              className="relative z-10 flex h-full w-1/2 items-center justify-center gap-1 rounded-full px-3 text-base font-semibold leading-[1.25] transition-[color,transform] duration-150 ease-out active:scale-[0.95]"
              aria-pressed={outcome === "yes"}
            >
              <span className="whitespace-nowrap">
                <span className="text-white/60">Yes </span>
                <span className={outcome === "yes" ? "text-white" : "text-white/60"}>
                  <NumberFlow value={selectedPerson.yesPercent} suffix="%" trend={0} />
                </span>
              </span>
              <span className="rounded-full bg-white/10 px-2.5 py-1 text-xs font-semibold leading-[1.25] text-white">
                <NumberFlow
                  value={yesMaxLeverage}
                  suffix="×"
            ...
            >
              <span className="whitespace-nowrap">
                ...
                <span className={outcome === "no" ? "text-white" : "text-white/60"}>
                  <NumberFlow
                    value={100 - selectedPerson.yesPercent}
                    suffix="%"
                    trend={0}
                  />
                </span>
              ...
              <span className="rounded-full bg-white/10 px-2.5 py-1 text-xs font-semibold leading-[1.25] text-white">
                <NumberFlow
                  value={selectedPerson.noLeverage}
                  suffix="×"
                  format={{ minimumFractionDigits: 0, maximumFractionDigits: 0 }}
                  trend={0}
                />
          ...
          {/* Leverage (buy only) */}
          {side === "buy" && (
            <SmartLeverageSection
              maxLeverage={activeOddMaxLeverage}
              leverageIdx={leverageIdx}
              onLeverageIdxChange={handleLeverageIdxChange}
              isDragging={isLeverageDragging}
              onDraggingChange={setIsLeverageDragging}
            />
          )}
          ...
          {orderType === "market" ? (
            ...
            <div className="flex w-full flex-col gap-2 rounded-3xl border border-transparent bg-[linear-gradient(90deg,#2a2a2a_0%,#262626_100%)] p-4 transition-[border-color,background-color,background-image] duration-150 ease-out focus-within:border-white/20 focus-within:bg-transparent focus-within:[background-image:none]">
              <label
